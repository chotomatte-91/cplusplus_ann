...
__global__ void dotProduct(float* vector, float* __restrict__ matrix, float* output, uint length)
{
	...
	__shared__ float partialResult[BLOCKSIZE][BLOCKSIZE];
	...
	if (vecIndex < length && row < length)
	{
		...
		{
			if(index < BLOCKSIZE)
				partialResult[threadIdx.y][index] += partialResult[threadIdx.y][index + i];
			__syncthreads();
		}
		//for (unsigned i = blockDim.x; i > 0; i >>= 1) {
		//	if (threadIdx.x < i) {
		...
		__syncthreads();
		if(threadIdx.x == 0)
			atomicAdd(output + row, partialResult[threadIdx.y][0]);
	}
